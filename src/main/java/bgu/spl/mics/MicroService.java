...
public abstract class MicroService implements Runnable {
    ...
    private ConcurrentHashMap<Class<? extends Message>, Callback> callBacksList;
    private Message m; // temp message for callback in run
    ...
    public MicroService(String name) {
        this.name = name;
        this.callBacksList = new ConcurrentHashMap<>();
    }
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        if(callback != null && !callBacksList.containsKey(type)) {
            callBacksList.put(type, callback);
        }
            ...
            synchronized (this){
                while (!terminated) {
                    try {
                        ...
                        if (callBacksList.get(m.getClass()) != null) {
                            callBacksList.get(m.getClass()).call(m);
                        }
                    }
                }
            }
